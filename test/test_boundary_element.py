...
DUNAVANT1_TRI_QUAD = np.array([[1.0, 1.0 / 3.0, 1.0 / 3.0]], dtype=np.float64)
...
DUNAVANT5_TRI_QUAD = np.array(
    [
        [0.225000000000000, 0.333333333333333, 0.333333333333333],
        [0.132394152788506, 0.470142064105115, 0.470142064105115],
        [0.132394152788506, 0.059715871789770, 0.470142064105115],
        [0.132394152788506, 0.470142064105115, 0.059715871789770],
        [0.125939180544828, 0.101286507323456, 0.101286507323456],
        [0.125939180544828, 0.797426985353087, 0.101286507323456],
        [0.125939180544828, 0.101286507323456, 0.797426985353087],
    ],
    dtype=np.float64,
)
...
def _triangle_current_density_reference(
    ...
    for i, (i0, i1, i2) in enumerate(triangles):
        ...
        out[i] = (s[i0] * (n1 - n2) + s[i1] * (n2 - n0) + s[i2] * (n0 - n1)) / (2.0 * area)
    return out
...
def test_triangle_mesh_serial_vs_parallel(quad):
    ...
    a_serial = np.column_stack(
        cfsem.vector_potential_triangle_mesh(obs, nodes, triangles, s, par=False, quad=quad)
    )
    a_parallel = np.column_stack(
        cfsem.vector_potential_triangle_mesh(obs, nodes, triangles, s, par=True, quad=quad)
    )
