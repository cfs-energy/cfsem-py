...
phi2 = sigma1 * dx * (F0(a1,b1,c) + F0(a2,b2,c) + F0(a3,b3,c)) + sigma1*F1(a1,b1,c) + sigmaEP*F1(a2,b2,c) + sigmaEP*F1(a3,b3,c) + sigmaOB*F2(a2,b2,c) - sigmaOB*F2(a3,b3,c);

%% 6. Compute the derivative d(phi)/dx (using the finite-difference limit)
Hx_sigma1_finite_difference = -(phi2 - phi1) / dx;
...
integrand_Hx_sigma1 = @(x,y) (- x .* (sigma1*x)) ./ ((x.^2+y.^2+c^2).^(3/2));
Hx_num = (1/(4*pi*mu0)) * integral2(integrand_Hx_sigma1, 0, a, @(x) 0, @(x) (b/a)*x, 'RelTol',1e-14,'AbsTol',1e-14);

fprintf('Hx_sigma1 from direct numerical integration: %12.16g\n', Hx_num);

fprintf('difference :  %12.16g\n', Hx_sigma1_finite_difference - Hx_num);
