...
B = [a, b, 0];
C = [0, 0, c]; 
D = [du*a/sqrt(a^2 + b^2), du*b/sqrt(a^2 + b^2), c];
E = [du*a/sqrt(a^2 + b^2), du*b/sqrt(a^2 + b^2), 0];
F = [a, du*b/sqrt(a^2 + b^2), 0]; % E projected on AB
P = [du*a/sqrt(a^2 + b^2), 0, 0];
...
phi2 = sigma1 * ((du*a/sqrt(a^2+b^2)) * (F0(a1,b1,c) + F0(a2,b2,c) + F0(a3,b3,c) + F0(a4,b4,c)) + F1(a1,b1,c) + F1(a2,b2,c) + F2(a3,b3,c) - F2(a4,b4,c));

%% 6. Compute the derivative d(phi)/dx (using the finite-difference limit)
dphidu = -(phi2 - phi1) / du;

% from previous
dphidx = (sigma1/(4*pi*mu0)) * (c*atan(a*b/(Dac^2 + c*Dabc)) + (a*b/(Dabc*(Dabc-b)))*((b+c-Dabc)+(c*(b+c)*(c-Dabc))/Dab^2));

% extract dphi/dy component by subtracting the dphidx component
Hy_sigma1_finite_difference = (sqrt(a^2 + b^2)*dphidu - a*dphidx)/b;
