...
% The shifted potential for the sigma2 term is approximated by
%   phi_shift_sigma2 = F(a1,b1,c) + F(a2,b2,c) - F(a3,b3,c).
sigmaEP = sigma2 * dot((P-E)/norm(P-E), [0;1;0]);
sigmaOB = sigma2 * dot((O-B)/norm(O-B), [0;1;0]);
phi1 = sigma2 * F2(a,b,c);
phi2 = sigma2*dy*(F0(a1,b1,c) + F0(a2,b2,c) + F0(a3,b3,c) + F0(a4,b4,c) + ...
	F0(a5,b5,c)) + sigma2 * (F2(a1,b1,c) + F2(a2,b2,c) + F1(a3,b3,c)) - ...
	(sigmaEP * (F1(a4,b4,c) + F1(a5,b5,c)) + 
		sigmaOB*(F2(a4,b4,c) - F2(a5,b5,c)));

%% 6. Compute the derivative d(phi)/dx (using the finite-difference limit)
Hy_sigma2_finite_difference = -(phi2 - phi1) / dy;
...
%% 7. Then the sigma2 contribution to Hx is
fprintf('Hy_sigma2 from puzzling triangles: %12.14g\n', Hy_sigma2_finite_difference);

%% 8. For comparison: Direct numerical integration of the sigma2 part.
% For sigma2, the integrand for φ is: (sigma2*x) / sqrt(x^2+y^2+c^2).
% Its contribution to Hx is then:
integrand_Hy_sigma2  = @(x,y) ( -y .* ( sigma2*y  ) ) ./ ((x.^2+y.^2+c^2).^(3/2));

Hy_num  = (1/(4*pi*mu0)) * integral2(integrand_Hy_sigma2, 0, a, @(x) 0, @(x) (b/a)*x, 'RelTol',1e-14,'AbsTol',1e-14);

fprintf('Hy_sigma2 from direct numerical integration: %12.14g\n', Hy_num);

fprintf('difference :  %12.14g\n', Hy_sigma2_finite_difference - Hy_num);
